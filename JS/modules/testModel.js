...
export async function testModel(model, inputData, normalizationData) {
    ...
    const preds = tf.tidy(() => {
        ...
        const pred = model.predict(normPredictionInput);
        ...
        const norm = unNormPreds.dataSync();
        return norm
    });
    ...
    const predictedVsIndexArray = tensorPredictedInputData.indexedPredictionInput.map((d, i) => {
        return { x: d.index, y: preds[i] }
    }).sort((a, b) => a.x - b.x);


    //training output data in an array
    const training = originalPoints.map(d => d.y)
